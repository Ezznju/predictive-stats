...
import { fetchTrades, fetchHolders, fetchGammaEvents } from '@/lib/pulse/polymarket-data';
...
export async function GET(
  ...
  const result = await withPulseCache(PULSE_KEYS.marketStats(conditionId), async () => {
    ...
    // 4. Get top holders
    let holders = await fetchHolders(conditionId);
