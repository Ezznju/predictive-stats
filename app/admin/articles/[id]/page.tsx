  ...
  if (!article) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={() => router.push('/admin/articles')} className="text-brand-red mt-2">← Back to articles</button>
      </div>
    );
  }
    ...
    <div className="max-w-5xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Pull Quote</label>
            <textarea
        ...
        <div className="space-y-6">
          <div className="bg-surface-raised rounded-xl border border-surface-border p-4">
            ...
            <input type="text" value={form.slug} onChange={(e) => updateField('slug', generateSlug(e.target.value))} className="w-full bg-surface border border-surface-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-red transition-colors font-mono" />
          </div>
          <div className="bg-surface-raised rounded-xl border border-surface-border p-4">
            ...
            {form.featuredImage && (
              <div className="relative rounded-lg overflow-hidden mb-2">
                ...
                <button onClick={() => updateField('featuredImage', '')} className="absolute top-2 right-2 p-1 bg-black/50 rounded-full text-white"><X className="w-3 h-3" /></button>
              </div>
            )}
            <input type="url" value={form.featuredImage} onChange={(e) => updateField('featuredImage', e.target.value)} className="w-full bg-surface border border-surface-border rounded-lg px-3 py-2 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-brand-red transition-colors" />
          </div>
          <div className="bg-surface-raised rounded-xl border border-surface-border p-4">
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Category</label>
            <select value={form.categorySlug} onChange={(e) => updateField('categorySlug', e.target.value)} className="w-full bg-surface border border-surface-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-red transition-colors">
              {categories.map((cat) => <option key={cat.id} value={cat.slug} className="bg-surface">{cat.name}</option>)}
            </select>
          </div>
          <div className="bg-surface-raised rounded-xl border border-surface-border p-4">
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Author</label>
            <select value={form.authorId} onChange={(e) => updateField('authorId', e.target.value)} className="w-full bg-surface border border-surface-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-red transition-colors">
              {authors.map((a) => <option key={a.id} value={a.id} className="bg-surface">{a.name}</option>)}
            </select>
          </div>
          <div className="bg-surface-raised rounded-xl border border-surface-border p-4">
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Tags</label>
            <input type="text" value={form.tags} onChange={(e) => updateField('tags', e.target.value)} className="w-full bg-surface border border-surface-border rounded-lg px-3 py-2 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-brand-red transition-colors" />
          </div>
          <div className="bg-surface-raised rounded-xl border border-surface-border p-4">
          ...
          <div className="bg-surface-raised rounded-xl border border-surface-border p-4 space-y-3">
            ...
            <div>
              ...
              <input type="text" value={form.seoTitle} onChange={(e) => updateField('seoTitle', e.target.value)} className="w-full bg-surface border border-surface-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-brand-red transition-colors" />
              <p className="text-[10px] text-slate-600 mt-0.5">{form.seoTitle.length}/60</p>
            </div>
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-600 mb-1">Meta Description</label>
              <textarea value={form.metaDescription} onChange={(e) => updateField('metaDescription', e.target.value)} rows={3} className="w-full bg-surface border border-surface-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-brand-red transition-colors resize-none" />
              <p className="text-[10px] text-slate-600 mt-0.5">{form.metaDescription.length}/160</p>
            </div>
          </div>
        </div>
      </div>
